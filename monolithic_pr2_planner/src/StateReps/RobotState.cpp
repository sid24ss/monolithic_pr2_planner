...
RobotState::RobotState(ContBaseState base_state, ContObjectState r_c_obj_state,
    ...
    DiscObjectState r_disc_obj_state(r_c_obj_state);
    DiscObjectState l_disc_obj_state(l_c_obj_state);

    bool ik_success = computeRobotPose(r_disc_obj_state, l_disc_obj_state,
        *this, new_robot_pose_ptr);
    int c = 0;
    while (!ik_success && c < 10000) {
        ...
        ik_success = computeRobotPose(r_disc_obj_state, l_disc_obj_state,
            *this, new_robot_pose_ptr);
        c++;
    }
    if (!ik_success) {
    ...
    m_left_obj_state = m_left_arm.getObjectStateRelBody();
    m_right_obj_state = m_right_arm.getObjectStateRelBody();
...
bool RobotState::computeRobotPose(const DiscObjectState& disc_r_obj_state,
                                 const DiscObjectState& disc_l_obj_state,
                                 const RobotState& seed_robot_pose,
                                 RobotPosePtr& new_robot_pose,
                                 bool free_angle_search){
    ContObjectState r_obj_state = disc_r_obj_state.getContObjectState();
    ContObjectState l_obj_state = disc_l_obj_state.getContObjectState();

    RightContArmState seed_r_arm = seed_robot_pose.right_arm();
    LeftContArmState seed_l_arm = seed_robot_pose.left_arm();

    KDL::Frame r_obj_frame;
    r_obj_frame.p.x(r_obj_state.x());
    r_obj_frame.p.y(r_obj_state.y());
    r_obj_frame.p.z(r_obj_state.z());
    r_obj_frame.M = KDL::Rotation::RPY(r_obj_state.roll(), 
                                     r_obj_state.pitch(),
                                     r_obj_state.yaw());
    KDL::Frame r_obj_to_wrist_offset = seed_robot_pose.right_arm().getObjectOffset();
    KDL::Frame r_wrist_frame = r_obj_frame * r_obj_to_wrist_offset;

    KDL::Frame l_obj_frame;
    l_obj_frame.p.x(l_obj_state.x());
    l_obj_frame.p.y(l_obj_state.y());
    l_obj_frame.p.z(l_obj_state.z());
    l_obj_frame.M = KDL::Rotation::RPY(l_obj_state.roll(), 
                                     l_obj_state.pitch(),
                                     l_obj_state.yaw());
    KDL::Frame l_obj_to_wrist_offset = seed_robot_pose.right_arm().getObjectOffset();
    KDL::Frame l_wrist_frame = l_obj_frame * l_obj_to_wrist_offset;
    ...
    bool use_right_arm = (m_planning_mode == PlanningModes::RIGHT_ARM ||
                          ...
                          m_planning_mode == PlanningModes::RIGHT_ARM_MOBILE ||
                          m_planning_mode == PlanningModes::DUAL_ARM_MOBILE || 
                          m_planning_mode == PlanningModes::TWO_ARM);
    bool use_left_arm = (m_planning_mode == PlanningModes::LEFT_ARM ||
                          m_planning_mode == PlanningModes::DUAL_ARM ||
                          m_planning_mode == PlanningModes::LEFT_ARM_MOBILE ||
                          m_planning_mode == PlanningModes::DUAL_ARM_MOBILE || 
 ...
 */
ContObjectState RobotState::getLeftObjectStateRelMap() const {
...
ContObjectState RobotState::getLeftObjectStateRelMap(ContBaseState base) const {
    std::vector<double> angles;
    SBPLArmModelPtr arm_model;
    KDL::Frame offset;
...
/*! \brief Gets the pose of the object in map frame.
 */
ContObjectState RobotState::getRightObjectStateRelMap() const {
    std::vector<double> angles;
    SBPLArmModelPtr arm_model;
    KDL::Frame offset;
...
ContObjectState RobotState::getRightObjectStateRelMap(ContBaseState base) const {
    std::vector<double> angles;
    SBPLArmModelPtr arm_model;
    KDL::Frame offset;
...
/*! \brief Gets the pose of the left object in base link frame.
 */
DiscObjectState RobotState::getLeftObjectStateRelBody() const {
    return m_left_obj_state;
}

/*! \brief Gets the pose of the right object in base link frame.
 */
DiscObjectState RobotState::getRightObjectStateRelBody() const {
    return m_right_obj_state;
}
...
int RobotState::numInterpSteps(const RobotState& start, const RobotState& end){
    ContObjectState r_start_obj = start.getRightObjectStateRelBody();
    ContObjectState r_end_obj = end.getRightObjectStateRelBody();

    ContObjectState l_start_obj = start.getLeftObjectStateRelBody();
    ContObjectState l_end_obj = end.getLeftObjectStateRelBody();

    ContBaseState start_base = start.base_state();
    ContBaseState end_base = end.base_state();

    int r_obj_interp_steps = ContObjectState::numInterpSteps(r_start_obj, r_end_obj);    
    int l_obj_interp_steps = ContObjectState::numInterpSteps(l_start_obj, l_end_obj);    
    ...
    int rot_steps = static_cast<int>(dbase_theta/ContObjectState::getRPYResolution());
    int dist_steps = static_cast<int>(d_base/ContBaseState::getXYZResolution());

    int base_interp_steps = max(rot_steps, dist_steps);

    ROS_DEBUG_NAMED(MPRIM_LOG, "rot steps %d, dist_steps %d", rot_steps, dist_steps);

    // get the max out of l_arm, r_arm and the base
    int num_interp_steps = max(base_interp_steps, r_obj_interp_steps);
    num_interp_steps = max(num_interp_steps, l_obj_interp_steps);
...
bool RobotState::workspaceInterpolate(const RobotState& start, const RobotState& end,
    ...
    vector<ContObjectState> l_interp_obj_steps;
    vector<ContObjectState> r_interp_obj_steps;
    ...
    l_interp_obj_steps = ContObjectState::interpolate(l_start_obj, l_end_obj, 
                                                    num_interp_steps);
    r_interp_obj_steps = ContObjectState::interpolate(r_start_obj, r_end_obj, 
                                                    num_interp_steps);
    interp_base_steps = ContBaseState::interpolate(start_base, end_base, 
                                                   num_interp_steps);
    assert(l_interp_obj_steps.size() == interp_base_steps.size());
    assert(r_interp_obj_steps.size() == interp_base_steps.size());
    // ROS_DEBUG_NAMED(MPRIM_LOG, "size of returned interp %lu", l_interp_obj_steps.size());

    // should at least return the same start and end poses
    if (num_interp_steps < 2){
        assert(l_interp_obj_steps.size() == 2);
        assert(r_interp_obj_steps.size() == 2);
    } else {
        assert(l_interp_obj_steps.size() == static_cast<size_t>(num_interp_steps));
        assert(r_interp_obj_steps.size() == static_cast<size_t>(num_interp_steps));
    }

    for (size_t i=0; i < l_interp_obj_steps.size(); i++){
        ...
        if (!computeRobotPose(r_interp_obj_steps[i], l_interp_obj_steps[i], seed, new_robot_state)){
            return false;
        }
